...
RegisterNetEvent('esx_holdup3:currentlyRobbing')
AddEventHandler('esx_holdup3:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holdup3:killBlip')
AddEventHandler('esx_holdup3:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_holdup3:setBlip')
AddEventHandler('esx_holdup3:setBlip', function(position)
...
RegisterNetEvent('esx_holdup3:tooFar')
AddEventHandler('esx_holdup3:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_holdup3:robberyComplete')
AddEventHandler('esx_holdup3:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_holdup3:startTimer')
AddEventHandler('esx_holdup3:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_holdup3:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_holdup3:tooFar', store)
			end
		end
	end
end)
