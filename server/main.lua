...
RegisterServerEvent('esx_holdup3:tooFar')
AddEventHandler('esx_holdup3:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_holdup3:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('esx_holdup3:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('esx_holdup3:robberyStarted')
AddEventHandler('esx_holdup3:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.PoliceNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('esx_holdup3:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('esx_holdup3:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('esx_holdup3:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('esx_holdup3:robberyComplete', _source, store.reward)
				...
				TriggerClientEvent('esx:showNotification', _source, _U('min_four_police', Config.PoliceNumberRequired))
			end
		else
